...
-- Exposed so `.emmyrc.json`'s `workspace.library` can reference installed
-- plugins' types via `$NVIM_DATA` (env vars are inherited by LSP servers
-- Neovim spawns, e.g. emmylua_ls) without hardcoding an absolute/user path.
...
-- Shorthand for building a GitHub repo URL, used by every `vim.pack.add()`
-- call across `lua/plugins/*.lua`.
